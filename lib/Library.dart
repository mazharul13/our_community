...
class dbCOn {
    ...
    try{
      await getConnection().then((conn) async {
        ...
        await conn.query(sql).then((result) {
          ...
          for (var r in result) {
            m = {"MEMBER_ID": r["MEMBER_ID"], "MEMBER_NAME": r["MEMBER_NAME"], "CONTACT_NO": r["CONTACT_NO"],
              "ADDRESS": r["ADDRESS"], "BLOOD_GROUP": r["BLOOD_GROUP"],
              "ENTRY_DT1": r["ENTRY_DT1"],"ENTRY_DT1_FORMATTED": r["ENTRY_DT1_FORMATTED"],
              "MEMBER_FNAME": r["MEMBER_FNAME"], "MEMBER_SINCE": r["MEMBER_SINCE"],
        ...
        await conn.query(sql).then((result) {
          ...
          for (var r in result) {
            m = {"ISSUE_ID": r["ISSUE_ID"], "ISSUE_TITLE": r["ISSUE_TITLE"],
              "ISSUE_DATES": r["ISSUE_DATES"], "STATUS": r["STATUS"]};
            map1.add(m);
          }
        });
      });
  ...
  Future<List<Map>> getIssues_payment_info(var sql) async {
    ...
    try{
      await getConnection().then((conn) async {
        ...
        await conn.query(sql).then((result) {
          ...
          for (var r in result) {
            m = {"ISSUE_ID": r["ISSUE_ID"], "ISSUE_TITLE": r["ISSUE_TITLE"],
              "ISSUE_DATES": r["ISSUE_DATES"], "AMOUNT_COLLECTED": r["AMOUNT"]
